...
export const signup = catchAsync(
  async (req: Request<{}, {}, SignupAuthInput>, res: Response) => {
...
export const login = catchAsync(
  async (req: Request<{}, {}, LoginAuthInput>, res: Response) => {
    const { email, password } = req.body;

    const { accessToken, refreshTokenString } = await authService.login(
      email,
      password,
    );
...
export const refresh = catchAsync(async (req: Request, res: Response) => {
  ...
  const { newAccessToken, newRefreshTokenString } =
