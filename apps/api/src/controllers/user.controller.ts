...
export const deleteUser = catchAsync(async (req: Request<DeleteUserInput>, res: Response) => {
...
export const updateMe = catchAsync(async (req: Request<{}, {}, UpdateMeInput>, res: Response) => {
  const userId = req.userId;
...
export const beta_updateAvatarUrl = catchAsync(
  async (req: Request<{}, {}, beta_UploadAvatarUrlInput>, res: Response) => {
    ...
    const uploadResult = await userService.beta_uploadAvatarUrl(userId, avatarUrl);
    res.status(HTTP_STATUS.OK).json({ success: true, data: uploadResult });
  }
);
